...
#
# This has to run before anything opens a connection, so it sits above the
# settings_default import rather than with the rest of the demo's settings at
...
if sys.platform == "darwin":
    try:
        ...
        class _DemoConnection(sqlean.dbapi2.Connection):
        ...
        def _connect(*args, **kwargs):
            kwargs.setdefault("factory", _DemoConnection)
...
# ---------------------------------------------------------------------------
# evennia-calendar demo
# ---------------------------------------------------------------------------

INSTALLED_APPS += ["evennia_calendar"]

# One game hour per ten real seconds. Fast enough to watch the watches turn
# (every 40s) and the day roll over (every 4 minutes) without sitting waiting,
# and slow enough that the clock's one-second tick still sees every hour
# rather than skipping past several between ticks.
TIME_FACTOR = 360

# Wall clock rather than accumulated uptime, so the demo keeps moving while the
# server is stopped rather than freezing where it was left.
TIME_IGNORE_DOWNTIMES = True

# The library's one setting. Optional — leave it out and the world starts in
# year 1000.
