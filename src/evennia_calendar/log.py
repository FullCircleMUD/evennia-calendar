...
"""Logging shim for evennia-calendar.

Every line the library emits goes to its own ``calendar.log``, co-located
with Evennia's other logs under ``settings.LOG_DIR``, so debugging the
calendar means reading one file rather than picking it out of the main
server log.
...
the server logs use — so a calendar line and a `server.log` line can be read
...
bootstrapped — ``calendar_log`` is a silent no-op. The import is lazy and an
...
_LOG_FILENAME = "calendar.log"
_VALID_LEVELS = ("INFO", "WARN", "ERROR")


def calendar_log(message: str, level: str = "INFO", trace: bool = False) -> None:
    """Emit one line to ``calendar.log``.
